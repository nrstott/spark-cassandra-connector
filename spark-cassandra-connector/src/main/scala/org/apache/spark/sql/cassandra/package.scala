...
package object cassandra {
  ...
  implicit class CSQLContext(sqlContext: SQLContext) {
    ...
    def addKeyspaceLevelReadConf(
        conf: ReadConf,
        keyspace: String,
        cluster: String = getDefaultCluster): Unit = {
      readConfSettings.addKeyspaceLevelConf(keyspace, cluster, conf)
    }

    /** Add cluster level read configuration settings */
    def addClusterLevelReadConf(conf: ReadConf, cluster: String = getDefaultCluster): Unit = {
    ...
    def removeKeyspaceLevelReadConf(keyspace: String, cluster: String = getDefaultCluster): Unit = {
      readConfSettings.removeKeyspaceLevelConf(keyspace, cluster)
    }

    /** Remove cluster level read configuration settings */
    def removeClusterLevelReadConf(cluster: String = getDefaultCluster): Unit = {
    ...
    def addClusterLevelWriteConf(conf: WriteConf, cluster: String = getDefaultCluster) : Unit = {
    ...
    def removeKeyspaceLevelWriteConf(keyspace: String, cluster: String = getDefaultCluster) : Unit = {
      writeConfSettings.removeKeyspaceLevelConf(keyspace, cluster)
    }

    /** Remove cluster level write configuration settings */
    def removeClusterLevelWriteConf(cluster: String = getDefaultCluster) : Unit = {
    ...
    /** Add cluster level write configuration settings */
    def addCassandraConnConf(conf: CassandraConnectorConf, cluster: String = getDefaultCluster) : Unit = {
      connConfSettings.addClusterLevelConf(cluster, conf)
    }

    /** Remove cluster level write configuration settings */
    def removeClusterLevelCassandraConnConf(cluster: String = getDefaultCluster) : Unit = {
    ...
    def getCassandraConnConf(cluster: String = getDefaultCluster) : CassandraConnectorConf = {
    ...
    private def tableIdentWithCluster(tableIdent: TableIdent) : TableIdent = {
      if (tableIdent.cluster.nonEmpty)
        ...
        TableIdent(tableIdent.table, tableIdent.keyspace, Option(getDefaultCluster))
    }
  }
